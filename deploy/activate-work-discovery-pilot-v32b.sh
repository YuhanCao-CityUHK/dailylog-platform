...
EVENT_FUSION_PILOT_FLAG="DAILY_ASSISTANT_EVENT_FUSION_PILOT_USERIDS"
...
# Compute the Event pilot list through the application's real parsing rules,
# remove only the exact approved userid, and preserve every remaining entry and
# its order. This happens before any backup or mutation.
EVENT_FUSION_PILOTS_AFTER="$(
  cd "$APP_DIR"
  sudo -u dailylog env \
    DOTENV_CONFIG_QUIET=true \
    EXCLUDED_EVENT_PILOT_USERID="$PILOT_USERID" \
    node --import tsx --input-type=module <<'TS'
import { CONFIG } from "./src/infra/config.ts";
const excluded = String(process.env.EXCLUDED_EVENT_PILOT_USERID ?? "");
...
const before = CONFIG.assistant.eventFusionPilotUserids;
if (!before.includes(excluded)) {
  throw new Error("approved employee is not in the pre-activation Event pilot list");
}
const after = before.filter((userid) => userid !== excluded);
if (CONFIG.assistant.eventFusionEnabled && after.length === 0) {
  throw new Error("cannot leave enabled Event fusion without any pilot users");
}
if (after.some((userid) => !/^[A-Za-z0-9._:@+-]+$/.test(userid))) {
  throw new Error("Event pilot userid cannot be written safely to the environment file");
}
process.stdout.write(after.join(","));
TS
)"
...
set_env_value "$APP_DIR/.env" "$EVENT_FUSION_PILOT_FLAG" "$EVENT_FUSION_PILOTS_AFTER"
...
(
  ...
  sudo -u dailylog env \
    DOTENV_CONFIG_QUIET=true \
    EXPECTED_EVENT_FUSION_PILOTS="$EVENT_FUSION_PILOTS_AFTER" \
    node --import tsx --input-type=module <<'TS'
import { CONFIG, validateConfig } from "./src/infra/config.ts";
const expected = "example-user-4";
const expectedEventPilots = String(process.env.EXPECTED_EVENT_FUSION_PILOTS ?? "")
  .split(",")
  .filter(Boolean);
...
if (CONFIG.assistant.eventFusionPilotUserids.includes(expected)) {
  throw new Error("approved work-discovery pilot must be excluded from strict Event fusion");
}
if (JSON.stringify(CONFIG.assistant.eventFusionPilotUserids) !== JSON.stringify(expectedEventPilots)) {
  throw new Error("remaining Event fusion pilot list changed during activation");
}
console.log(JSON.stringify({
  enabled: true,
  pilotCount: 1,
  eventFusionPilotCount: expectedEventPilots.length,
  maxCandidates: 50,
  llmReady: true,
}));
TS
...
printf 'event_fusion_excluded_pilot=%s\n' "$PILOT_USERID" >> "$BACKUP_DIR/DEPLOYED_HOTFIX.activated"
