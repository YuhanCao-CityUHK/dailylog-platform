...
BASELINE_RELEASE="20260902-daily-assistant-work-discovery-v32"
BASELINE_COMMIT="1802a81250421e8e56868a0cf748389e2a55075e"
EXPECTED_MAIN_MIGRATION="10"
EXPECTED_CONTEXT_MIGRATION="8"
...
verify_hotfix_scope() {
  ...
  expected="$({
    printf '%s\n' \
      'public/app.js' \
      'public/index.html' \
      'src/assistant/bounded-parallel.ts' \
      'src/assistant/candidate-service.ts' \
      'src/assistant/interactions/interaction-generation-service.ts' \
      'src/assistant/interactions/prompts/task-discovery-v1.ts' \
      'src/assistant/routes.ts' \
      'tests/assistant-interactions.test.ts' \
      'tests/assistant-performance.test.ts' \
      'tests/candidate-service.test.ts'
  } | sort -u)"
  cmp -s \
    <(printf '%s\n' "$expected") \
    <(sort -u "$UPLOAD_DIR/release-files.txt") || {
      echo "v32b payload must contain exactly the reviewed latency hotfix and its tests" >&2
      return 1
    }
  local expected_existing
  expected_existing="$({
    printf '%s\n' \
      'public/app.js' \
      'public/index.html' \
  ...
  cmp -s \
    <(printf '%s\n' "$expected_existing") \
    <(sort -u "$UPLOAD_DIR/existing-files.txt") || {
      echo "v32b existing-file manifest is outside the reviewed scope" >&2
      return 1
    }
  cmp -s \
    <(printf '%s\n' 'src/assistant/bounded-parallel.ts') \
    "$UPLOAD_DIR/new-files.txt" || {
      echo "v32b may add only the reviewed bounded-parallel helper" >&2
      return 1
    }
  test ! -s "$UPLOAD_DIR/removed-files.txt" || {
    echo "v32b may not remove application files" >&2
    return 1
  }
}

verify_runtime_work_discovery_off() {
  (
    cd "$APP_DIR"
...
if (CONFIG.assistant.workDiscoveryEnabled !== false) {
  throw new Error("v32b baseline must have work discovery disabled");
}
if (CONFIG.assistant.workDiscoveryPilotUserids.length !== 0) {
  throw new Error("v32b baseline pilot list must be empty");
}
TS
  )
}
...
verify_baseline() {
  ...
  grep -qx "candidate_commit=${BASELINE_COMMIT}" /opt/dailylog/DEPLOYED_HOTFIX
  grep -qx "work_discovery=installed-disabled" /opt/dailylog/DEPLOYED_HOTFIX
  (
    cd "$APP_DIR"
...
restore_backup() {
  ...
  wait_for_url "$PUBLIC_BASE/healthz" 15 || rollback_rc=1
  wait_for_url "$PUBLIC_BASE/readyz" 15 || rollback_rc=1
...
test "$CANDIDATE_COMMIT" != "$BASELINE_COMMIT" || {
  echo "v32b candidate commit must differ from the deployed v32 commit" >&2
  exit 1
}
...
echo "[1/8] Validating manifests, v32 disabled baseline, and production guards"
test -d "$APP_DIR"
test -d "$PAYLOAD_DIR"
...
test "$BASELINE_MAIN_MIGRATION" = "$EXPECTED_MAIN_MIGRATION"
test "$BASELINE_CONTEXT_MIGRATION" = "$EXPECTED_CONTEXT_MIGRATION"
...
test "$STAGED_CONTEXT_MIGRATION" = "$BASELINE_CONTEXT_MIGRATION" || {
  echo "interaction batching hotfix must not change the context database schema" >&2
  false
}
...
printf '%s\n' \
  ...
  "scope=daily-assistant-work-discovery-v32b-latency-hotfix" \
